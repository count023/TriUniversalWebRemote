...
(hope you have one. This code will not work as a local file opened in a browser due to CrossSiteScripting-restrictions - if so, you need to change it, for example by using a hidden image-tag updated with the desired URI or so)
